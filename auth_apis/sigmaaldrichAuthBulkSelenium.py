...
class SigmaaldrichAuthBulkSelenium():
    locale = 'CH'
    lang = 'en'
    getLinkUrl = "https://www.sigmaaldrich.com/GB/en/product/"
    def __init__(self,filename,locale,lang,userName=None,password=None):
        self.productLinks = {}
        self.proxyList = []
        self.usedProxy = []
        self.maxProxy = 1000
        ...
        self.logFile = datetime.now().strftime("SigmadrichAuthBulkSelenium_%Y_%m_%d-%I-%M-%S_%p")
        with open(f'{self.logFile}.log','w') as f:
            f.write('SIGMA AUTH BULK SELENIUM LOGS\n')
        ...
        self.getLinkUrl = f"https://www.sigmaaldrich.com/{self.locale}/{self.lang}/products/"
    ...
    #     self.linksVisited.append(link)
    #     print(f"Link: {link}")
    #     self.writeLogs(f"Category Link: {link}\n")

    #     try:
    #         check = self.driver.find_elements(By.XPATH,value='//a[contains(@href,"/product/")]')
    #         if len(check)>0:
    #             self.getProductLinks(link)
    #             print(f"Total Product Links acquired: {len(self.productLinks)}")
    #             self.writeLogs(f"Total Product Links acquired: {len(self.productLinks)}\n")

    #             return
    #         else:
    #             try:
    #                 tempCat = set(link.get_attribute('href') for link in self.driver.find_elements(By.XPATH,value='//a[contains(@href,"/products/")]'))
    ...
    #         return
    #     cat = list()
    #     cat = cat + [cat for cat in tempCat if '/cookie-consent/' not in cat]
    #     for i in cat:
    #         if i not in self.linksVisited:
    #             self.categories(i)
    ...
    def getProductLinks(self):
        ...
        url = 'https://www.sigmaaldrich.com/sitemap_index.xml'
        ...
        for count,siteLink in enumerate(links):
            if '_prod_' not in str(siteLink):
                continue
            r = requests.get(str(siteLink).replace('<loc>','').replace('</loc>',''),headers=headers, timeout=12)
            print(r.status_code)
            en = BeautifulSoup(r.text,'lxml')
            en_Links_1 = en.find_all('loc')

            
            for i in en_Links_1:
                if '/product/' in str(i):

                    url = str(i).replace('<loc>','').replace('</loc>','').split('/')
                    #print(url)
                    templocale = url[3]
                    templang = url[4]
                    if templang !=self.lang or templocale != self.locale:
                        data = str(i).replace('<loc>','').replace('</loc>','').replace(f'/{templocale}/',f'/{self.locale}/').replace(f'/{templang}/',f'/{self.lang}/')
                        #print(f'THSI IS: {data}')
                    else:
                        data = str(i).replace('<loc>','').replace('</loc>','')
                    if data != self.productLinks.get(data):
                        self.productLinks[data] = 1
            print(f'Link: {siteLink}link number traversed:{count+1}, Total Products gathered: {len(self.productLinks)}')
            self.writeLogs(f'Link: {siteLink}link number traversed:{count+1}, Total Products gathered: {len(self.productLinks)}\n')
            #break #test        
    ...
    def main(self):
        try:
            ...
            print(e)
            self.driver.close()
# t = SigmaaldrichAuthBulkSelenium('SIGMALDRICHBULK','CH','en','aaron.ueckermann@axiomdata.io','AxiomPassword123!')
# t.main()
