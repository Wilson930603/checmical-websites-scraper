...
class faustAuthPriceSpider(BaseSpider):
    ...
    start_urls = ['https://www.faust.ch/shop/Liquid_Handling/Microfluidics/Hoses_and_connections/-way_stopcock-Discofix$B$einfo5466_lang_UK.htm']
    history = []
    custom_settings={
        'USER_AGENT':''
    }
    def __init__(self,start_urls=['https://www.faust.ch/shop/Liquid_Handling/Microfluidics/Syringe_pumps/Accessories_for_Microlab_700$B$einfo185138_lang_UK.htm?UID=550062258554000000000000&OFS=0'],userName=None,password=None):
    ...
    def parseProducts(self, response):
        # print(re.findall(r"SetUserStatus(.*?);",response.text)) #check logged in or no
        try:
...
#     date = datetime.now().strftime("IMPROVED_faust_auth_price_%Y_%m_%d-%I-%M-%S_%p")
...
#         process.crawl(faustAuthPriceSpider,
#         start_urls=['https://www.faust.ch/shop/Liquid_Handling/Microfluidics/Hoses_and_connections/-way_stopcock-Discofix$B$einfo5466_lang_UK.htm'],
