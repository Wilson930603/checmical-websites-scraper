...
class VwrAuthPageSelenium():
    getLinkUrl = 'https://us.vwr.com/store/category/adhesives/3617154'
    locale = 'us'
    ...
    def getProductLinks(self):
        self.driver.get(self.getLinkUrl+"?sortOrder=2")
        sleep(1)
        products = self.driver.find_elements(By.XPATH,"//*[contains(@id,'productLink')]//a")
        for product in products:
            self.productLinks.append(product.get_attribute("href"))
    ...
    def main(self):
        try:
            ...
            print(f'VWR PAGE: Total Product page link: {len(self.productLinks)}')
            for count,links in enumerate(self.productLinks):
                self.driver.get(links)
                sleep(1)
                temp = self.getProduct(links)
                if temp == None:
                    continue
                for tempData in temp:
                    returnedData.append(tempData)
                print(f'VWR PAGE: Pages Scraped: {count+1}')
            self.driver.close()
            print(returnedData)
...
# t = VwrAuthPageSelenium('https://us.vwr.com/store/category/adhesives/3617154','us',userName, passwrod)
