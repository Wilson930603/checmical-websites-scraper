...
class sigmaaldrichPageSpider(scrapy.Spider):
    locale = 'CH' #example : CZ, FR, #please check website for available locale. usually https://sigmaaldrich.com/CH/en -> meaning Locale = CH, language  = en
    ...
    def parse(self, response):
        ...
        if('/products/' in baseUrl):
            yield scrapy.Request(url=baseUrl,callback=self.parseCategories,headers=self.headers,meta={'mainCat':''})
        else:
            try:
