...
class sigmaaldrichBulkSpider(scrapy.Spider):
    filename = 'sigmaaldrichBulk'
    ...
    ################ SETTING HERE ####################
    start_urls=["https://www.sigmaaldrich.com/"+locale+"/"+lang]
    name = 'sigmaaldrichBulk'
    history = []
    payload_history = []
                ...
                if(categories[catKey]['__typename']=='AemTopNavItem'):
                    ...
                    if(not isMenu):
                        ...
                        if('/products/' in url):
                            ...
                            # print(urlCreator)
                            urlCreator = urlCreator.replace("/EN/us/EN/us","/EN/us").replace("/US/en/US/en","/US/en")
                            # print(urlCreator)
                            #urlCreator = 'https://www.sigmaaldrich.com/US/en/products/chemistry-and-biochemicals/stable-isotopes'
                            yield scrapy.Request(url=urlCreator,callback=self.parseCategories,headers=self.headers,meta={'mainCat':title,'url':urlCreator})
                            # break #testing
    ...
    def parseCategories(self,response):
        ...
        if('/products' in baseUrl):
            ...
            mainCat = response.meta['mainCat']
            print(baseUrl+"->"+url)
            yield scrapy.Request(url=url,callback=self.parseCategories,headers=self.headers,meta={'mainCat':mainCat,'url':url},dont_filter=True)
     
    def parseProducts(self,response):
        baseUrl = response.url
        payload = response.meta['payLoad']
        mainCat = response.meta['mainCat']
        if('sigmaaldrich.com/api' in baseUrl):
            try:
                ...
                for product in products:
                    ...
                    yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parseDetail,body=rawJsonz,method='POST',headers=self.headers,meta={'mainCat':mainCat,'productUrl':urlCreator,'payload':payloadz},dont_filter=True,priority=9)
                ...
                if(len(products)==20):
                    try:
                        ...
                        yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parseProducts,body=rawJsonNext,method='POST',headers=self.headers,meta={'mainCat':mainCat,'payLoad':payloadNext},dont_filter=True,priority=2)
                    except Exception as e:
                        pass
            ...
            yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parseProducts,body=payload,method='POST',headers=self.headers,meta={'mainCat':mainCat,'payLoad':payload},dont_filter=True,priority=2)
    ...
    def parseDetail(self,response):
        ...
        if('sigmaaldrich.com/api' in baseUrl):
            ...
            rawJsonz = json.loads(payloadz,strict=False)
            rawJsonz = json.dumps(rawJsonz)
            yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parseDocument,body=rawJsonz,method='POST',headers=self.headers_price,meta={'p':p,'productNumber':detail['erpProductKey'],'brand':detail['brand']['erpKey'],'catalogId':detail['catalogId']},dont_filter=True,priority=12)
        else:
            rawJsonz = response.meta['payload']
            urlCreator = response.meta['productUrl']
            mainCat = response.meta['mainCat']
            yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parseDetail,body=rawJsonz,method='POST',headers=self.headers,meta={'mainCat':mainCat,'productUrl':urlCreator,'payload':rawJsonz},dont_filter=True,priority=9)
    ...
    def parseDocument(self,response):
        ...
        rawJsonz = json.loads(payloadz,strict=False)
        rawJsonz = json.dumps(rawJsonz)
        # if(payloadz not in self.payload_history):
        #     self.payload_history.append(payloadz)
        #     yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parsePrice,body=rawJsonz,method='POST',headers=self.headers_price,meta={'p':p,'productNumber':productNumber,'brand':brand},dont_filter=True,priority=13)
        yield scrapy.Request('https://www.sigmaaldrich.com/api',callback=self.parsePrice,body=rawJsonz,method='POST',headers=self.headers_price,meta={'p':p,'productNumber':productNumber,'brand':brand,'payload':rawJsonz},dont_filter=True,priority=13)
