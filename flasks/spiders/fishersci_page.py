...
class fishersciPageSpider(scrapy.Spider):
    ...
    daPage = 'https://www.fishersci.co.uk/gb/en/products/I9C8KVO5/antibody-production-purification.html'
    #######################################################
    if(regionConfig == 'gb'):
    ...
    def parse(self, response):
        ...
        if('/shop/' in respUrl):
            prodCode = ''
            try:
                prodCode = re.findall(r"(?:.(?!/))+$",respUrl)[0]
                prodCode = prodCode.replace("/","")
            except:pass
            yield scrapy.Request(respUrl,callback=self.parseDetails,headers=self.headers_json,meta={'catName':'','groupUrl':'','prodCode':str(prodCode)})
        else:
            try:
                identifier = re.findall(r"products/(.*?)/",respUrl)[0]
                print(identifier)
                ...
                yield scrapy.Request(urlCreator,callback=self.parseProducts,headers=self.headers_json,meta={'firstRun':False,'catName':''})
                # break #testing
            except:
        ...
        for product in products:
            try:
                ...
                for subProduct in subProducts:
                    ...
                    yield scrapy.Request(urlProduct,callback=self.parseDetails,headers=self.headers_json,meta={'catName':catName,'groupUrl':groupUrl,'prodCode':str(subProduct)},dont_filter=True)
            except:
                pass
            # break #testing
