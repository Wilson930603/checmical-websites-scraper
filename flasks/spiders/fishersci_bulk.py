...
class fishersciBulkSpider(scrapy.Spider):
    filename = 'fishersciBulk'
    locale = 'ch' # please check https://www.fishersci.co.uk/gb/en/change-country.html for 'locale' & lang
    lang = 'en'  # example : belgium -> fishersci.be/be/fr/home.html meaning locale = be, lang = fr .
                       # example : United Kingdom -> fishersci.co.uk/gb/en meaning locale = gb, lang = en . 
                       # for lang, please check the website for availabilities, sometimes it have fr,it,en in one region.
    #######################################################
    if(locale == 'gb'):
        baseUrl = 'https://www.fishersci.co.uk/'+locale+'/'+lang
        shopUrl = 'https://www.fischersci.co.uk'
    else:
        baseUrl = 'https://www.fishersci.'+locale+'/'+locale+'/'+lang
        shopUrl = 'https://www.fishersci.'+locale
    name = 'fishersciBulk'
    pages = []
    ...
    def parse(self, response):
        if(self.locale == 'gb'):
            self.baseUrl = 'https://www.fishersci.co.uk/'+self.locale+'/'+self.lang
            self.shopUrl = 'https://www.fischersci.co.uk'
        elif(self.locale == 'us'):
            self.locale = 'ch'
            self.baseUrl = 'https://www.fishersci.ch/ch/en'
            self.shopUrl = 'https://www.fishersci.ch'
        else:
            self.baseUrl = 'https://www.fishersci.'+self.locale+'/'+self.locale+'/'+self.lang
            self.shopUrl = 'https://www.fishersci.'+self.locale
        respUrl = response.url
        print(respUrl)
        categoryUrl = respUrl.replace("home.html","products.html")
        yield scrapy.Request(categoryUrl,callback=self.parseCategories,headers=self.headers_normal)
    ...
    def parseProducts(self, response):
        ...
        for product in products:
            try:
                ...
                for subProduct in subProducts:
                    ...
                    yield scrapy.Request(urlProduct,callback=self.parseDetails,headers=self.headers_json,meta={'catName':catName,'groupUrl':groupUrl,'prodCode':str(subProduct)},priority=1)
            except:
                pass
            # break #testing
        ...
        if(firstRun):
            ...
            for x in range(0,maxRange,30):
                ...
                yield scrapy.Request(urlNext,callback=self.parseProducts,headers=self.headers_json,meta={'firstRun':False,'catName':catName},priority=-1)
            
            refinements = rawJson['refinements']
            for refinement in refinements:
                if(refinement['name']=='CATEGORY'):
                    subs = refinement['refinementElements']
                    for sub in subs:
                        check = False
                        try:
                            check = sub['navigable']
                        ...
                        if(check):
                            try:
                                regex = re.findall(r"([^>]*)\/",sub['url'])[0]
                                # print(regex)
                                urlCreator = self.baseUrl + '/catalog/service/browse/products?storeId=10652&offset=0&Ns=default&identifier='+regex
                                yield scrapy.Request(urlCreator,callback=self.parseProducts,headers=self.headers_json,meta={'firstRun':True,'catName':catName},priority=-5)
                            except:pass
    ...
    def parseDetails(self, response):
        ...
        # print(data)
        yield scrapy.Request('https://www.fishersci.ch/shop/products/documents/'+str(prodCode),callback=self.parseDocuments,headers=self.headers_normal,meta={'data':data},priority=9)
